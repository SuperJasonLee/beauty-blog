"""Crawler module: uses opencli to search intimate area plastic surgery news."""
import json
import logging
import subprocess
import sys
...
DATA_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "crawled" / "intimate-aesthetics-news"
...
SOURCES = [
    {
        ...
        "command": [
            ...
            "labiaplasty OR vaginoplasty OR genital aesthetic surgery outcomes 2025 2026",
            "--limit", "10", "-f", "json"
        ],
    },
    {
        ...
        "command": [
            "opencli", "pubmed", "search",
            "penile lengthening OR scrotal reconstruction male genital aesthetic surgery 2026",
            "--limit", "10", "-f", "json"
        ],
    },
    {
...
def extract_zhihu_articles(data: list[dict]) -> list[dict]:
    articles = []
    for item in data:
        # Strip HTML tags from author name
        ...
        articles.append({
            ...
            "content_markdown": f"**Author:** {author}\n**Votes:** {item.get('votes', 0)}",
            "image_urls": [],
            "crawled_at": datetime.now(timezone.utc).isoformat(),
        })
    return articles
...
def crawl_source(source: dict, crawled_urls: set) -> list[dict]:
    ...
    if source["name"] in ("pubmed", "pubmed_male"):
...
def save_results(articles: list[dict]) -> Path:
    ...
    out_file = DATA_DIR / f"intimate_aesthetics_news_{ts}.json"
