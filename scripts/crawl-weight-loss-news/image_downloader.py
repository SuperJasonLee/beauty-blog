...
IMAGES_DIR = Path(__file__).resolve().parent.parent.parent / "static" / "images" / "posts" / "weight-loss-aesthetics-2026-06"
...
def download_images_from_article(article: dict, date_str: str, counter_start: int) -> tuple[dict, int]:
    ...
    for url in image_urls:
        ...
        if result:
            downloaded[url] = f"/images/posts/weight-loss-aesthetics-2026-06/{filename}"
            counter += 1
...
def process_crawled_file(json_path: Path) -> dict:
    ...
    for article in articles:
        ...
        for img_url in article.get("image_urls", []):
            local = f"/images/posts/weight-loss-aesthetics-2026-06/{generate_filename(img_url, date_str, counter)}"
