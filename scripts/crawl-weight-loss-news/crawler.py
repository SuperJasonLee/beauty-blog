"""Crawler module: uses opencli to search and extract weight loss aesthetics news."""
...
SOURCES = [
    {
        "name": "pubmed",
        "command": ["opencli", "pubmed", "search", "weight loss aesthetic medicine OR body contouring OR liposuction OR semaglutide obesity 2026", "--limit", "10", "-f", "json"],
    },
    {
        "name": "zhihu",
        "command": ["opencli", "zhihu", "search", "减肥医美 溶脂 吸脂 2026", "--limit", "10", "-f", "json"],
    },
    {
        "name": "google",
        "command": ["opencli", "web", "read", "--url", "https://www.google.com/search?q=weight+loss+aesthetic+medicine+news+2026", "-f", "json"],
    },
]
...
def save_results(articles: list[dict]) -> Path:
    ...
    out_file = DATA_DIR / f"weight_loss_news_{ts}.json"
